<script lang="ts">
	...
	import CbctWorkspace from '$lib/components/cbct/CbctWorkspace.svelte';
	...
	// A non-CBCT study (X-ray / panoramic / photo / IOS) reached on this route by
	// bookmark, typed address, or a hand-edited id would mount CbctWorkspace on data
	// it can't parse (a JPEG, or a mesh) — failing the volume load AND exposing a
	// "Run AI Segmentation" CTA that would bill a paid CBCT seg on the wrong file.
	...
	const wrongModality = $derived(!!study && !studyBelongsOnRoute(study.modality, 'cbct'));
	...
	// entirely — a bookmarked / history / typed `/cbct/…` link must not mount the
...
{#if patient && study && !wrongModality && auth.threeDEnabled}
	...
	<!-- Key on study id: navigating between 3D studies on the same route (e.g. via
	     the History menu) reuses this page, and the workspace loads its volume in
	     onMount — without remounting, study B would keep showing study A's volume. -->
	{#key study.id}
		<CbctWorkspace {patient} {study} patientId={patientId!} />
	{/key}
{:else}
	<!-- Spinner doubles as the wrong-modality transient state: the $effect above is
	     navigating away to the correct viewer, so we just avoid mounting CbctWorkspace
